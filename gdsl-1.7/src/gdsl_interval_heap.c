...
static int 
vti (gdsl_element_t *e);

static void
raw_dump (const gdsl_interval_heap_t heap);
...
static int vti (gdsl_element_t *e) 
{
    return (int)(*(int *)(((void *)e) + 28));
}
...
raw_dump (const gdsl_interval_heap_t heap)
{
	...
	{
	    curr1 = current;
	    for (i = 0; i < length && curr1 <= heap->card; i++) 
		{
		    fprintf (stderr, "%4lu ", LAST_INDEX (curr1));
		    if (curr1 % 2 == 0)
			{
			    fprintf (stderr, "| ");
			}
	    ...
	    for (i = 0; i < length && curr1 <= heap->card; i++) 
		{
		    fprintf (stderr, "%4d ", vti (heap->nodes[ LAST_INDEX (curr1)]));
		    if (curr1 % 2 == 0) 
			{
			    printf ("| ");
			}
